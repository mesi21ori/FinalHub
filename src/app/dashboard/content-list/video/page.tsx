...
const fetchVideos = async () => {
    try {
        const response = await fetch("/api/platfromadmin/contentlist/video");
...
const VideoListPage: React.FC = () => {
    ...
    const handleToggleActive = async (id: number) => {
        ...
        const newIsActiveStatus = !data.find(video => video.id === id)?.isActive;
        ...
        try {
          const response = await fetch(`/api/platfromadmin/contentlist/activate/${id}`, {
            ...
            body: JSON.stringify({
              isActive: newIsActiveStatus, // Send the new active status
            }),
          });
          ...
          const updatedContent = await response.json(); // Handle this if needed
      
          // Update the local state with the response data if necessary
          // Uncomment to merge response updates
          // setData((prevData) =>
          //   prevData.map((video) =>
          //     video.id === updatedContent.updatedVideo.id ? updatedContent.updatedVideo : video
          //   )
          // );
      
        } catch (error) {
          console.error('Error during video activation toggle:', error);
          ...
          alert('Failed to toggle active status for the video. Please try again.');
        }
      };
                ...
                renderRow={(row) => {
                    ...
                    return (
                        <>
                            ...
                            <td>{row.numberOfViews}</td>  {/* Safe access to numberOfViews */}
