...
handler.post(async (req: NextApiRequest & { file: Express.Multer.File }, res: NextApiResponse) => {
  ...
  // Basic input validation
  if (!email || !password || !username || !firstName || !lastName || !gender || !dateOfBirth || !phoneNumber) {
    return res.status(400).json({ message: 'Email, password, username, firstName, lastName, gender, dateOfBirth, and phoneNumber are required' });
  }
