...
//     const status = req.url?.includes('reject') ? 'REJECTED' : 'APPROVED';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const message = `Your application has been REJECTED. Please contact support if you have any questions.`;
