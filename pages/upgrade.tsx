...
const UpgradePage: React.FC = () => {
  ...
  const [email, setEmail] = useState<string>(''); // Store user's email

  useEffect(() => {
    const storedUserId = localStorage.getItem('userId');
    const storedEmail = localStorage.getItem('email'); // Retrieve email from local storage
    setUserId(storedUserId || '');
    setEmail(storedEmail || ''); // Default to an empty string if no email is found
    ...
    <div>
      <header className="w-full bg-[#3C2A21] text-[#E5E5CB] p-4">
        <div className="flex justify-between items-center">
          ...
          <Link href="/content" className="text-[#E5E5CB]">Back</Link>
          <Link href="/dashboard" className="text-[#E5E5CB]">Account</Link>
        </div>
      </header>
