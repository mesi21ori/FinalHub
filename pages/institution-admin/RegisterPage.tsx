...
export default function RegisterPage({ onSubmit, onBack }: RegisterPageProps) {
  ...
  const [gender, setGender] = useState(''); // Use string for gender
  const [agreed, setAgreed] = useState(false);
  const [errorMessage, setErrorMessage] = useState('');

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!adminUsername || !adminPassword || !adminEmail || !adminFirstName || !adminLastName || !phoneNumber || !gender) {
      setErrorMessage("All fields must be filled out");
      return; // Prevent submission if fields are empty
    }
    setErrorMessage(''); // Clear any previous error message
      ...
      <form onSubmit={handleSubmit} className="w-full max-w-3xl bg-[#E5E5CB] p-4 md:p-6 rounded-lg shadow-md flex flex-col md:flex-row gap-8">
        ...
        <div className="w-full md:w-1/2 flex flex-col items-center">
          <div className="w-full mt-4 space-y-4">
            ...
            <div>
              <label htmlFor="phoneNumber" className="block text-[#3C2A21]">Phone Number</label>
              <input
                type="tel"
                id="phoneNumber"
                value={phoneNumber}
            ...
            <div>
              <label htmlFor="gender" className="block text-[#3C2A21]">Gender</label>
              <select
                id="gender"
                value={gender}
