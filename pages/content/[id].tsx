...
//     const [comments, setComments] = useState<Comment[]>([]);
//     const [newComment, setNewComment] = useState<string>('');
//     const [reportReason, setReportReason] = useState<string>(''); // Default to empty string
//     const [additionalDetails, setAdditionalDetails] = useState<string>(''); // For 'OTHER' reason
//     const [loading, setLoading] = useState<boolean>(true);
//     const [error, setError] = useState<string | null>(null);
//     const [reportingCommentId, setReportingCommentId] = useState<number | null>(null);
//     const router = useRouter();
...
//                 userId,
//                 reason,
//                 additionalDetails: reason === 'OTHER' ? additionalDetails : undefined,
//             });
//             console.log(response.data); // Log the success response
//             alert('Report submitted successfully!');
//             setReportingCommentId(null); // Close the reporting modal after submission
//         } catch (error: unknown) {
//             if (axios.isAxiosError(error)) {
//                 console.error('Error submitting report:', error.response || error.message);
//                 setError('Error submitting report. Please try again.');
...
const ContentDetail: React.FC = () => {
    ...
    const handleCommentSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            const response = await axios.post('/api/comments', { text: newComment, contentId, userId });
            setComments([...comments, { ...response.data, user: { username: localStorage.getItem('username') || 'Unknown' } }]);
            setNewComment('');
        } catch (error) {
            console.error('Error submitting comment:', error);
            setError('Error submitting comment. Please try again.');
    ...
    const handleReportSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            alert('Report submitted successfully!');
            setReportingCommentId(null); // Close reporting modal
